...
import math
import sys
import heapq # A* 알고리즘을 위한 모듈

# --- [1. 설정 값] ---
pygame.init()
WIDTH, HEIGHT = 1000, 800
screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Auto Drive - APF & A* Steering")
...
PATH_COLOR = (255, 105, 180) # 웨이포인트를 보여줄 핑크색 선
...
class Car:
    def __init__(self, x, y, goal_pos):
        ...
        self.max_hp = 500.0
        self.hp = self.max_hp
        self.waypoints = [] # [필수 수정] 누락되었던 웨이포인트 리스트 생성 변수 추가!
    ...
    def recalculate_path(self, obstacles):
        ...
        while open_set:
            iterations += 1
            if iterations > 2000: break # 과부하 방지
    ...
    def update(self, keys, obstacles): 
        ...
        if self.ai_enabled:
            # 1. 기본 목적지(또는 웨이포인트)로 향하는 인력 벡터 (Target Vector)
            if hasattr(self, 'waypoints') and self.waypoints and len(self.waypoints) > 0:
            ...
            # 2. 가상의 척력 벡터 계산 (Repulsive Vector)
            repulsive_x = 0.0
            repulsive_y = 0.0
            sensor_range = 150.0  # 척력이 작동하기 시작하는 가상 반경 (GTA의 감지 영역)
            ...
            for obs in obstacles:
                for i in range(len(obs) - 1):
                    ...
                    if dist < sensor_range and dist > 0:
                        # 가까울수록 제곱비례로 강력하게 밀어내는 척력 공식
                        force_magnitude = ((sensor_range - dist) / sensor_range) ** 2 * 450.0
            ...
            # 3. 인력과 척력을 합성하여 [최종 조향 벡터] 생성
            final_dx = desired_dx + repulsive_x
            final_dy = desired_dy + repulsive_y

            # 최종 벡터를 기준으로 목표 각도 계산
            target_angle = math.degrees(math.atan2(-final_dy, final_dx)) - 90
            angle_diff = (target_angle - self.angle + 180) % 360 - 180
            
            # 4. 부드러운 유선형 조향 제어 (오차에 비례해 부드럽게 복원)
            steering_speed = angle_diff * 0.15
            steering_speed = max(-5, min(5, steering_speed))
            self.angle += steering_speed

            # 속도 조절 (척력이 걸려 위험하거나 전방에 벽이 가까울 땐 제동)
            obstacle_ahead = self.check_ray_collision(obstacles)
            current_kmh = self.speed * 10
            
            if obstacle_ahead or math.hypot(repulsive_x, repulsive_y) > 120:
                if self.speed > 0: self.speed -= self.acceleration * 1.5 
            else:
                target_spd = self.target_val if abs(angle_diff) < 25 else self.target_val * 0.4
    ...
    def draw(self, surface, zoom):
        # AI 주행 중일 때 찾은 네비게이션 웨이포인트 경로 그리기
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    if event.button == 1:
                        ...
                        if len(drawing_points) > 1: 
                            ...
                            if car: car.recalculate_path(obstacles) # 마우스 떼서 벽을 그릴 때 즉시 경로 재계산
                        drawing_points = []
        ...
        if state == "MENU":
            ...
            if draw_button(pygame.Rect(WIDTH-150, 20, 130, 40), "운전하기", mx, my, color=BLUE if (setup_start and setup_end) else GRAY):
                for e in events:
                    if e.type == pygame.MOUSEBUTTONDOWN and (setup_start and setup_end):
                        ...
                        car.recalculate_path(obstacles) # 최초 실행 시 경로 연산 실행
            else:
                for e in events:
            ...
            if is_erasing:
                ...
                if len(obstacles) != old_len and car:
                    car.recalculate_path(obstacles) # 우클릭으로 장애물을 지웠을 때도 실시간 경로 재계산
